...
public class ProductDao {
    ...
    public static void delete(String id) {
        ...
        DbOperations.setDataOrDelete(query, "Product Delete Sucessfully");
    }
    ...
    public static ArrayList<Product> fillterProductByName(String name,String category) {
        ArrayList<Product> list = new ArrayList<>();
        try {
            ResultSet rs = DbOperations.getData("select * from product where name like '%"+name+"%' category = '"+category+"'");
            while(rs.next()) {
                Product product = new Product();
                product.setName(rs.getString("name"));
                list.add(product);
